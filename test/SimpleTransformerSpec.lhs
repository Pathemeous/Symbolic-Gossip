...
The first three tests (explained below) describe instances of higher-order knowledge and aren't 
...
tests 4-8 encode the basic requirements of a transformer for a Gossip problem and 9-10 encode general instances of 
...
spec = do
        ...
        it "simpTrf 1: agents can reason about other agents' knowledge" $ do 
            eval (afterSimple 4 [(0,1),(1,2)]) (K "3" (has 3 2 0)) `shouldBe` True
        it "simpTrf 2: agents can reason about the limits of other agents' knowledge" $ do 
            eval (afterSimple 3 [(0,1)]) (K "0" (Neg (has 3 2 1))) `shouldBe` True 
        it "simpTrf 3: all agents know that all are experts after the correct call sequence" $ do 
        ...
        it "simpTrf 4: no faulty knowledge in initial model" $ do 
            eval (simpleGossipInit 2) (K "0" (Neg (has 2 1 0))) `shouldBe` True
        it "simpTrf 5: call shares secrets between agents" $ do 
            eval (afterSimple 2 [(0,1)]) (Conj [has 2 1 0, has 2 0 1]) `shouldBe` True 
        it "simpTrf 6: call sequence shares secrets between agents" $ do 
            eval (afterSimple 3 [(0,1),(1,2)]) (has 3 2 0) `shouldBe` True 
        it "simpTrf 7: no faulty experts" $ do 
            eval (afterSimple 3 [(0,1)]) (Disj [expert 3 i | i <- [0..3]]) `shouldBe` False 
        it "simpTrf 8: all are experts after the correct call sequence" $ do 
